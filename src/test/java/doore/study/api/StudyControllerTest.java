...
public class StudyControllerTest extends IntegrationTest {
    ...
    class StudyCreateTest {
        ...
        @Test
        @DisplayName("[성공] 정상적으로 스터디를 생성한다.")
        ...
        @ParameterizedTest
        @DisplayName("[실패] 필수값이 입력되지 않은 경우 스터디 생성에 실패한다.")
        @CsvSource({
        ...
        void createStudy_필수값이_입력되지_않은_경우_스터디_생성에_실패한다_실패(final String name, final String description,
    ...
    @Test
    @DisplayName("[성공] 정상적으로 스터디를 삭제한다.")
    void deleteStudy_정상적으로_스터디를_삭제한다_성공() throws Exception {
        final String url = "/studies/" + study.getId();
        callDeleteApi(url, token).andExpect(status().isNoContent());
    }

    @Test
    @DisplayName("[성공] 정상적으로 스터디를 수정한다.")
    void updateStudy_정상적으로_스터디를_수정한다_성공() throws Exception {
    ...
    @Test
    @DisplayName("[성공] 정상적으로 스터디의 상태를 변경한다.")
    void changeStudyStatus_정상적으로_스터디의_상태를_변경한다_성공() throws Exception {
        final String url = "/studies/" + study.getId() + "/status?status=IN_PROGRESS";
        callPatchApi(url, study, token).andExpect(status().isNoContent());
    }

    @Test
    @DisplayName("[성공] 정상적으로 스터디를 종료한다.")
    void terminateStudy_정상적으로_스터디를_종료한다_성공() throws Exception {
